...
from .relation_prob import TwoWordPSDProbe
...
class BertPreTrainedModel(nn.Module):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, state_dict=None, cache_dir=None,
        ...
        """
        print('Instantiate a BertPreTrainedModel from a pre-trained model')
        ...
        tempdir = None
        resolved_archive_file = 'data/bert_ckpt'
...
class LXRTPretraining(BertPreTrainedModel):
    def __init__(self,
        ...
        # visual probe and textual probe
        # self.visual_probe=TwoWordPSDProbe()
        # self.textual_probe=TwoWordPSDProbe()
        #
        # self.mlp_proj=nn.Sequential(nn.Linear(in_features=36*36,out_features=512), # todo check 共用一个mlp还是各自一个？
...
class SSRP_Encoder(BertPreTrainedModel):
    """
    BERT model for classification.
    """
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, visual_feats=None,pos=None,
        ...
        if 'x' == self.mode:
            ...
            return (lang_output, visn_output), pooled_output
        elif 'l' in self.mode or 'r' in self.mode:
            return (lang_output, visn_output)

class SSRP_Probe(nn.Module):
    def __init__(self,sent_max_seq_len=36, vis_max_seq_len=36 ):
        super(SSRP_Probe, self).__init__()
        # super().__init__(config)
        # Configuration
        # self.config = config
        #
        ...
        # visual probe and textual probe
        self.visual_probe = TwoWordPSDProbe(model_dim=768)
        self.textual_probe = TwoWordPSDProbe(model_dim=768)
    ...
    def forward(self, lang_output, visn_output, pooled_output=None):

        vis_probe = self.visual_probe(visn_output)  # size is batch_size, max_seq_len, max_seq_len
        lang_probe = self.textual_probe(lang_output)  # todo 判断是否需要去掉[cls] [end]token？

        # 假设每个batch的前1/2是原始数据， 后1/2是数据增强后的数据
        # sent_target_probe size is batch_size,max_seq_len,max_seq_len
        B, _, _ = vis_probe.size()
        vis_probe_vec = self.mlp_proj(vis_probe.view(B, -1))
        lang_probe_vec = self.mlp_proj(lang_probe.view(B, -1))

        return vis_probe,lang_probe,vis_probe_vec,lang_probe_vec
